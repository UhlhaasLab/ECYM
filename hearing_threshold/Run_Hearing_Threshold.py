...
def run_hearing_threshold(device, buttonCodes, myLog, SUB, SUB_DIR):
    ...
    for rounds in range(2):
        ...
        if threshold_db is not None:
            out_path = os.path.join(SUB_DIR, 'round_' + str(rounds + 1) + '_hearing_threshold_1000.csv')
            with open(out_path, "w", newline='', encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["SUB", "threshold_db", "threshold_amplitude"])
                writer.writerow([SUB, f"{threshold_db:.2f}", f"{threshold_linear:.5f}"])
